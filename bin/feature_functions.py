...
import pyfeats
import numpy as np
import dermaml.features as feats
...
def feature_delegation(input_feat, hand_image):
    ...
    if input_feat == 'redness':
        features = find_function[input_feat](original_image, mask)

    else:   
        hsv_image = feats.remove_brightness(hand_image)
        ...
        if input_feat == 'lbp_glcm':
            ...
            if input_feat == 'pyfeats':
                ...
                hessian_image_a, _ = feats.detect_ridges(bw_image) 
...
def _normalized_redness(img:np.array, mask:np.array):
    '''
    ...
    red_values = [normalized_redness, relative_red_std]
...
def _lbp_glcm(hsv_image):
    # lbp_
    lbp_hist,lbp = feats.compute_lbp(hsv_image)
    enum_lbp = dict(enumerate(lbp_hist))
    lbp_features = {'lbp_'+str(k):v for k,v in enum_lbp.items()}

    # glcm_whole_image
    contrast, correlation, energy, homogeneity = feats.compute_glcm(hsv_image)
